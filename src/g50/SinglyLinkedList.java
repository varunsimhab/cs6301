...
public class SinglyLinkedList<T> implements Iterable<T> {
	...
	*/
	...
	// Rearrange the elements of the list by linking the elements at given index
    // Implemented by using an array to store hte number heads and tails and linking
    // array heads and tails at the end
    /**
     * Created by
     * Group 50
     *
	...
	public void multiUnzip(int k) {
        ...
        Entry<T>[] tail1 = (Entry<T>[]) Array.newInstance(head.getClass(), k);
		Entry<T>[] head1 = (Entry<T>[]) Array.newInstance(head.getClass(), k);
        ...
        while(state<k-1){
            head1[state+1] = tail1[state].next;
            tail1[state+1] = head1[state+1];
            state++;
        }

        Entry<T> c = tail1[state].next;
        ...
        while(state<k-1){
            tail1[state].next = head1[state+1];
            state++;
        }
        tail1[k-1].next = null;
	}
...
//			}
//		}
        System.out.println("Before MultiUnzip");
        lst.printList();
        lst.multiUnzip(2);
        System.out.println("After MultiUnzip");
		lst.printList();
//		lst.unzip();
//		lst.printList();
	}
